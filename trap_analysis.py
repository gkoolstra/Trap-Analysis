...
from static_surface import load_maxwell_data, select_domain
import common
...
def load_potentials(fn_resonator, fn_currentloop, fn_leftgate, fn_midgate, fn_rightgate):
    ...
    for name, fn in zip(potential_names, potential_fns):
        if fn[-4:] == '.fld':
            ...
            output.append({'name' : name, 'V' : array(V, dtype=float64),
                           'x' : array(x, dtype=float64), 'y' : array(y, dtype=float64)})
        else:
...
def fit_electron_potential(x, V, fitdomain=None, do_plot=False, plot_title=''):
    ...
    try:
        fr, ferr = dsfit.fit_parabola(x, V, fitparams=[0, -1, 0], domain=fitdomain, verbose=False)
    except:
        raise ValueError("Fit error!")

    if fitdomain is not None:
        fitdatax, fitdatay = dsfit.selectdomain(x, V, fitdomain)
    else:
        fitdatax, fitdatay = x, V

    # Get an estimate for the goodness of fit
    gof = dsfit.get_rsquare(fitdatay, dsfit.parabolafunc(fitdatax, *fr))

    if do_plot:
        plt.plot(x, V, '.k')
        plt.plot(fitdatax, dsfit.parabolafunc(fitdatax, *fr), '-r', lw=2.0, label='$r^2$ = %.2f'%(gof))
        plt.xlabel("$x$ ($\mu$m)")
...
def get_electron_frequency(fr, ferr, verbose=True):
    ...
    f = 1/(2*pi) * np.sqrt(-c['e']*2*fr[1]*1E12/c['m_e'])
    sigma_f = 1/(4*pi) * np.sqrt(-2*c['e']*1E12/(fr[1]*c['m_e'])) * ferr[1]
...
def sweep_electrode_voltage(x, y, potentials, coefficients, sweep_voltage, sweep_electrode_idx,
    ...
    for voltage in sweep_voltage:
        ...
        fr, ferr = fit_electron_potential(np.array(x[:]*1E6, dtype=float64), np.array(V[yminidx, :], dtype=float64),
                                          fitdomain=(fitdomain[0]*1E6, fitdomain[1]*1E6), do_plot=do_plot,
        ...
        fr, ferr = fit_electron_potential(np.array(x[:]*1E6, dtype=float64), np.array(V[yctridx, :], dtype=float64),
                                          fitdomain=(fitdomain[0]*1E6, fitdomain[1]*1E6), do_plot=do_plot,
